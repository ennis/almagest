...
impl<'a, T> BufSlice<'a, T>
{
	...
	pub fn as_read_slice(&self) -> &[T] {
		unsafe {
			slice::from_raw_parts(self.raw.map_ptr.offset(self.offset as isize) as *const T, self.size / mem::size_of::<T>())
		}
	}

	pub fn as_write_slice(&mut self) -> &mut [T] {
		unsafe {
			slice::from_raw_parts_mut(self.raw.map_ptr.offset(self.offset as isize) as *mut T, self.size / mem::size_of::<T>())
		}
	}
...
impl<'a, T> Buffer<'a, T>
{
	...
	fn get_write_mapping(&mut self) -> &mut [T] {
		unsafe {
			slice::from_raw_parts_mut(self.raw.map_ptr as *mut T, self.raw.size / mem::size_of::<T>())
		}
	}
...
impl BufferAllocator
{
	pub fn alloc_raw_buffer<'a>(
		...
		usage: BufferUsage,
		initial_data: Option<&[u8]>) -> RawBuffer<'a>
	{
